...
const (
	HostName      = "Distributed-Web"
	HostGroupName = "Prometheus-Dell-Demo"
)
...
func requestsGenerator() {
	for {
		resp, _ := http.Get("http://localhost:30000/analytics")
		_, _ = ioutil.ReadAll(resp.Body)
		_ = resp.Body.Close()
		resp, _ = http.Get("http://localhost:30000/distribution")
		_, _ = ioutil.ReadAll(resp.Body)
		_ = resp.Body.Close()
		resp, _ = http.Get("http://localhost:30000/sales")
		_, _ = ioutil.ReadAll(resp.Body)
		_ = resp.Body.Close()
